...
def main(suffix):
    ...
    digits_number = input(
        '\nCombien de digits .. :\n  - on tapote on tapote enter)\n\n>>> ')
    logging.info(digits_number + 'T OU PEPINOUUUUUUUUUUU  \n')
...
# -------------quick launch------------------------
    ...
    if (digits_number >= 4) and (digits_number <= 10):
        try:
            ...
            logging.info('RICHARDDDDDD C LHEURE DU DODO RICHHHHARRRRD')
            exit()
...
try:
    logging.info('--------------------------------')
    logging.info("ahhhhh t ou Gustave...")
    main(suffix)
